...
func Test_Pager(t *testing.T) {
	...
	assert.Equal(t, &Page{offset: 12288}, pgr.GetOrAlloc(12), "GetOrAlloc() should return newly allocated page")
	...
	assert.Equal(t, &Page{offset: 2048, buffer: []byte("hello world")}, pgr.Get(2), "Get() should return the set page")

	pgr.Set(20, []byte("foo bar"))
	assert.Equal(t, &Page{offset: 20480, buffer: []byte("foo bar")}, pgr.Get(20), "Get() should return the set page")

	pgr.Set(3, make([]byte, DEFAULT_PAGE_SIZE+10))
	assert.Equal(t, &Page{offset: 3072, buffer: make([]byte, DEFAULT_PAGE_SIZE)}, pgr.Get(3), "Set() should truncate the provided buffer down to the page size")
}
